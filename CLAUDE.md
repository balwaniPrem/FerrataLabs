...
- SEO settings, plugins and WordPress pages exist only on the server.
...
> physical file still carrying a stale `# START YOAST BLOCK` comment, and the sitemap it
> points at, `/sitemap_index.xml`, now resolves and is generated by Rank Math. The README's
> claim that it 404s is out of date.
...
Native WordPress posts at **ferratalabs.ai/blog**, reachable from a `Blog` nav item.
It is the one part of the site that does **not** come from the Next build, and the one
documented exception to §11's one-way rule.

### Why it is the exception

`parts/*.php` are generated snapshots of Next routes. A post list is dynamic and a post
is authored, not compiled, so it cannot be a snapshot. The resolution is a split by
*origin* rather than by file:

| Origin | Where it is written | Mirrored? |
|---|---|---|
| Agent-authored | `content/posts/<slug>.md` here, then pushed | one-way, push only |
| Human-authored | straight into wp-admin | no, and that is fine |

**Sync is one-way and only covers posts that started here.** A post written by hand in
wp-admin is legitimate and is never copied back. Pulling published posts down into the
repo is a later decision, deliberately deferred.

### Authoring and publishing

Posts go through a PR before they go out. §8 exists because the claims on this site have
to survive a second-meeting probe, and a post is mostly claims. Publishing from a chat
window skips the diff, which is how an unverified number escapes.
...
**Draft is the default; `--publish` is a separate deliberate act.** The first run stamps
`wpId` into the front matter and every run after that updates that exact post, so
re-publishing is idempotent and the script can never adopt a hand-written post by slug
collision. Format and front matter keys are in `content/posts/README.md`.

Credentials live in `~/Documents/Projects/ferratalabs.wp` (`WP_URL`, `WP_USER`,
`WP_APP_PASS`), mode `600`, outside both repos, mirroring how `push.sh` reads
`ferratalabs.sftp`. `WP_APP_PASS` is an **Application Password**, never an account
password. It is not committed and must never be pasted into a transcript.

### Theme files

| File | Role |
|---|---|
| `home.php` | blog index and, via `archive.php`, every archive |
| `archive.php` | thin `require` of `home.php` so one list serves both |
| `single.php` | one post |
| `assets/theme.css` | the blog styles, at the end |

> ⚠️ `home.php` at the theme root is the **posts index**, fixed by the WordPress template
> hierarchy. It has nothing to do with `parts/home.php`, which is the generated marketing
> homepage. The collision is unfortunate and unavoidable.

Layout is borrowed from `.phead`, `.sec`, `.wrap` and `.prose`, so the look comes from
the generated `assets/site.css` and cannot drift from the rest of the site. Only the post
list, meta line, long-form body and pagination are new, and they live in `theme.css`
because it is hand-maintained and the exporter never touches it. **`assets/site.css` is
generated and must not be hand-edited.** Bump `FERRATA_VERSION` whenever `theme.css`
changes or browsers keep the old file.

### Server-side setup

- Settings → Reading → *Posts page* must point at a page with the slug `blog`, or the
  nav item 404s.
- Rank Math owns per-post titles and descriptions. The theme emits meta only for routes
  in `ferrata_meta()`, and posts are deliberately not in it.
- Rank Math adds `post-sitemap.xml` to `/sitemap_index.xml` once the first post exists.

### There is no `/blog` in this app

The nav item exists in the theme's `header.php` only. Adding one to `components/Nav.tsx`
means building a renderer here for content WordPress already owns, and it would show only
repo-authored posts while missing every hand-written one. That divergence is intentional.
